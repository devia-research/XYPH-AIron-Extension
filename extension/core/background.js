...
Fournis le code converti complet.`
    ...
    setupContextMenus() {
        if (!chrome.contextMenus || typeof chrome.contextMenus.create !== 'function') {
            console.warn('[XYPH] contextMenus API indisponible. Vérifiez les permissions manifest.');
            return;
        }

        chrome.contextMenus.removeAll(() => {
            if (chrome.runtime.lastError) {
                console.warn('[XYPH] Impossible de nettoyer les menus contextuels:', chrome.runtime.lastError.message);
            }
